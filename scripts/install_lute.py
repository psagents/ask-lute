...
def update_dag_params(
    dag_path: str, partition: str, account: str, nodes: int, ntasks_per_node: int
) -> None:
...
def main() -> None:
    ...
    for wf_name in workflow_names:
        ...
        update_dag_params(
            full_workflow_path, partition, account, nodes, ntasks_per_node
        )
        logger.info(f"Workflow DAG ready: {full_workflow_path}")

    wf_list: str = " ".join(f"-W {w}" for w in workflow_names)
    fresh_flag: str = " -f" if args.fresh_install else ""
    dir_flag: str = f" -D {args.directory}" if args.directory else ""
    logger.info(
        "Workspace setup complete.\n"
        "To register workflows in the eLog, run:\n\n"
        f"  python post_elog_workflows.py -e {args.experiment}"
        f" -v {args.version}{fresh_flag}{dir_flag} {wf_list}\n\n"
        "A valid Kerberos ticket is required. If you do not have one, run:\n"
